...
### sidecar — 觀測伴侶 TUI(手)
[marcus/sidecar](https://github.com/marcus/sidecar) · ~1.0k★ · 活躍 · Go · MIT

在 CLI coding agent(Claude Code/Cursor/Codex/Gemini…)旁開一個分割 pane 的終端儀表板:即時 git diff、檔案樹、跨約 10 種 agent 的對話史、TD 任務監看、git worktree workspace(每 worktree 啟一個 agent)。
- 能力:🧠❌ ✋✅(worktree workspace + diff/檔案/任務即時儀表板 + 對話史) 🛣️❌(只讀取呈現,不是傳輸層)
- 主用途:單一 agent 的本機觀測伴侶 + worktree 平行開發;人在旁監督
- 注意:是觀測不是編排——不拆任務、不協調多 agent;worktree only(無容器/tmux 監督)
- 自架:本機 Go binary(`brew install marcus/tap/sidecar`)
- 來源:GitHub README + marcus.github.io/sidecar
...
### PromptX — 角色/記憶 context 平台(路)
[Deepractice/PromptX](https://github.com/Deepractice/PromptX) · ~3.7k★ · 活躍 · TypeScript · MIT

MCP 平台:靠對話把專業角色/persona、可建工具、長期認知記憶注入單一 AI app(Claude/Cursor/Trae)。
- 能力:🧠❌ ✋△(AgentX 每對話隔離 workspace + sandbox 除錯) 🛣️△(agent↔人 remote share + MCP streamable-http;**無 agent↔agent**)
- 主用途:給單一 agent 灌角色 + 工具 + 長期記憶(「Chat is all you need」)
- 注意:**單 agent 的 context/記憶平台,非多 agent 編排**——角色是「同一 agent 換身分」不是多 agent 協調;性質近 VCP 但更偏單體
- 自架:`npx @promptx/mcp-server` / Docker / 桌面客戶端
- 來源:GitHub README + promptx.deepractice.ai
...
以下**不是可跑的編排工具**,是理解多 agent 的素材或生態慣例。不列入上方工具紀錄,但選型/實作時值得讀。

### 方法論 / 文章

- **[Anthropic — How we built our multi-agent research system](https://www.anthropic.com/engineering/multi-agent-research-system)**:orchestrator-worker 的正統參考——主 agent 拆解、spawn 平行子 agent、綜合結果。硬核心得:平行省最多 90% 時間;**token 用量解釋約 80% 效能變異**(多 agent ~15× 成本,值得才 fan out);委派要明確;用 LLM-judge + rubric 評測。三軸都涉及(它是模式,不是單一工具)。
- **[知乎 — Multi-Agent System,一篇就夠了](https://zhuanlan.zhihu.com/p/1928636720796136414)**:上篇 Anthropic 文的中文導讀,拆 orchestrator-worker + 8 條實踐(按複雜度分配 agent 數、先廣後精、並行工具調用、checkpoint/resume、可觀測追蹤)。入門用;是二手整理,正本是上面 Anthropic 那篇。

### 生態慣例 / 素材(是 repo 但非工具)

- **[agentsmd/agents.md](https://github.com/agentsmd/agents.md)**(~23k★):`AGENTS.md` 慣例——放在 repo 根給 AI agent 讀的專案指引檔(「給 agent 的 README」),跨廠商標準,想統一 `CLAUDE.md`/`.cursorrules` 等。**是設定檔慣例不是工具**;上方多數編排器的 worker 會讀它來上手 codebase。
- **[elder-plinius/CL4R1T4S](https://github.com/elder-plinius/CL4R1T4S)**(~45k★):眾包的**系統提示洩漏檔案庫**(OpenAI/Anthropic/Cursor/Windsurf/Devin/Manus/Cline… 一廠一夾)。**不是工具**,但當「真 agentic 工具怎麼做拆解(腦)、工具鷹架、人機互動(路)」的一手素材很有用。
